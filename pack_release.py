...
def main():
    ...
    # 2b. Build ZablindUninstaller.exe
    print(f"\n--- 2b. Building ZablindUninstaller.exe ---")
    uninstaller_args = [
        "pyinstaller", "--onefile", "--noconsole",
        "--name=ZablindUninstaller",
        "--icon=docs/favicon.ico",
        "--hidden-import=psutil",
        "--hidden-import=accessible_output2",
        "--hidden-import=accessible_output2.outputs.auto",
        "--collect-all=psutil",
        "--collect-all=accessible_output2",
        "zablind_uninstaller.py"
    ]
    
    try:
        subprocess.run(uninstaller_args, cwd=root_dir, check=True)
        shutil.copy2(os.path.join(root_dir, "dist", "ZablindUninstaller.exe"), os.path.join(root_dir, "ZablindUninstaller.exe"))
        print("[PACKER] Successfully built ZablindUninstaller.exe")
    except Exception as e:
        print(f"[ERROR] Failed to build ZablindUninstaller.exe: {e}")
        speak("Packaging failed. Could not compile uninstaller.")
        sys.exit(1)
    ...
    # Clean installer/uninstaller build dirs
    ...
    if os.path.exists(spec_inst):
        os.remove(spec_inst)
    spec_uninst = os.path.join(root_dir, "ZablindUninstaller.spec")
    if os.path.exists(spec_uninst):
        os.remove(spec_uninst)
        
    speak("Zablind packaging completed successfully. Installer, uninstaller, and update zip files are ready.")
    print("\n==================================================")
    print("Build finished successfully!")
    print("Files created:")
    print(f"  - {os.path.join(root_dir, f'{installer_name}.exe')} (Installer)")
    print(f"  - {os.path.join(root_dir, 'ZablindUninstaller.exe')} (Uninstaller - upload to GitHub Releases!)")
